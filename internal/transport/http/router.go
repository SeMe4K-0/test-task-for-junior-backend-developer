...
func NewRouter(taskHandler *httphandlers.TaskHandler, docsHandler *swaggerdocs.Handler) *mux.Router {
	...
	api.HandleFunc("/tasks/materialize", taskHandler.Materialize).Methods(http.MethodPost)
	api.HandleFunc("/tasks/rules", taskHandler.ListRules).Methods(http.MethodGet)
	api.HandleFunc("/tasks/rules/{id:[0-9]+}", taskHandler.DeleteRule).Methods(http.MethodDelete)
